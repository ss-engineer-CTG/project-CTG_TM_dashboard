...
// チャートコンポーネントを動的インポート（パラレルロード）
const ProgressChart = dynamic(() => import('@/app/components/ProgressChart'), {
  ssr: false,
  loading: () => (
    <div className="dashboard-card h-80 flex items-center justify-center">
      <div className="animate-pulse text-text-secondary">進捗グラフをロード中...</div>
    </div>
  )
});

const DurationChart = dynamic(() => import('@/app/components/DurationChart'), {
  ssr: false,
  loading: () => (
    <div className="dashboard-card h-80 flex items-center justify-center">
      <div className="animate-pulse text-text-secondary">期間グラフをロード中...</div>
    </div>
  )
});
